...
* **Setup**: Place the script call (e.g., `pbDynamicTrainer("A")`) on Page 1 as a **Parallel Process**. Calling it this way automatically pulls from the *entire* available pool of standard trainers. It updates the overworld graphic instantly on map load and then automatically turns ON Self Switch `D`.
...
* **VIP Bosses**: The setup is exactly the same! Simply name your event "VIP" in the top-left of the editor, and `pbDynamicTrainer` will natively detect it and assign a boss from `DYNAMIC_VIPS`. On Page 3 (Condition: Self Switch `A`), add the `pbDefeatedVIP` script call so the player can extract.
