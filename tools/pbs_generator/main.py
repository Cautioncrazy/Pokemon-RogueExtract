...
class GeneratorThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Load the Map-to-Theme bridge generated by the Ruby script
            map_theme_overrides = {}
            bridge_path = os.path.join(os.path.dirname(__file__), "map_themes.json")
            ...
            for map_idx, map_id in enumerate(range(self.start_map, self.end_map + 1)):
                ...
                # Check for explicit tileset theme overrides for this specific map
                ...
                for floor_num in range(1, self.num_floors + 1):
                    ...
                    if tileset_override:
                        current_theme = tileset_override
                    else:
                        if self.apply_theme_all:
                    ...
                    override_str = " (Tileset Override)" if tileset_override else ""
                    filter_str = f" [Filter: {self.filter_category}={current_filter_val}]" if self.filter_category != "None" else ""
                    self.log_signal.emit(f"-> Floor {floor_num} | Theme: {current_theme}{override_str}{filter_str}")
