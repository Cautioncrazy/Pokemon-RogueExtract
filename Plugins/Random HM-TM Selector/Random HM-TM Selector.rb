...
# @param whiteList [Array<Symbol>, nil] Optional: An array of specific TM/HM IDs
#                                         to choose ONLY from. If nil, considers
#                                         all eligible TMs/HMs.
def pbGiveRandomTMorHM(whiteList = nil)
  eligible_items_for_selection = []
  
  # Determine the base pool of items to consider
  base_pool = []
  if whiteList # If a whitelist is provided, start with that
    base_pool = whiteList
  else # Otherwise, consider all TMs and HMs
    GameData::Item.each do |item_data|
      # Add all TMs
      if item_data.field_use == 5 # This is the internal value for TR (TM)
        base_pool.push(item_data.id)
      end
      # Add all HMs (ownership will be checked later)
  ...
  # Now filter the base_pool to get eligible items
  is_tm = proc { |item_data| item_data.field_use == 5 }
  is_hm = proc { |item_data| item_data.field_use == 4 }

  base_pool.each do |item_id|
    item_data = GameData::Item.try_get(item_id)
    next if item_data.nil?

    # Include TMs (no ownership check needed as they are reusable)
    if is_tm.call(item_data)
      eligible_items_for_selection.push(item_id)
    end

    # Include HMs, but only if the player doesn't already have them
    if is_hm.call(item_data)
      unless $player_given_hms.include?(item_id)
        eligible_items_for_selection.push(item_id)
      end
    end
  end
