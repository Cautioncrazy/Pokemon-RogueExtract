...
class Interpreter
  ...
  def pbSetAndStartDynamicTrainer(possible_types = nil, possible_names = nil, victory_switch = "A")
    ...
    if !event.instance_variable_defined?(:@dynamic_trainer_initialized) || !event.instance_variable_get(:@dynamic_trainer_initialized)
      ...
      if pre_assigned
        ...
        GameData::Trainer.each do |trainer|
          ...
          if (trainer.version / 100) == $game_map.map_id
            # Optionally filter by requested types if they were explicitly provided
            if !possible_types || (!possible_types.is_a?(Array) || possible_types.empty?) || possible_types.include?(trainer.trainer_type)
      ...
      event.instance_variable_set(:@dynamic_trainer_version, chosen_version)
    end
    ...
    # Retrieve the exact trainer state saved during setup
    chosen_type = event.instance_variable_get(:@dynamic_trainer_type)
    chosen_name = event.instance_variable_get(:@dynamic_trainer_name)
    chosen_version = event.instance_variable_get(:@dynamic_trainer_version) || RoguelikeExtraction.calculate_trainer_version
