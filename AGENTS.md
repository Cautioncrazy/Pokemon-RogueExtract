...
- **Implementation (`Plugins/Roguelike_Extraction/012_Alpha_Boss_Visuals.rb`):**
  ...
  - **HP Multi-bar UI:** Overrides the DBK boss health bar rendering logic (`refresh_hp` in `Battle::Scene::PokemonDataBox`) to create a multi-tier "fighting game" style bar system. It calculates the active tier dynamically based on the boss's native `hp_level` (capped at 6).
    - **Viewport Masking Refactor:** To ensure the custom multi-tier bar perfectly inherits DBK's slanted clipping mask and to prevent texture squishing, the active bar is drawn directly onto the vanilla DBK canvas (`@hpBar.bitmap`) at 100% width using `stretch_blt`. DBK's native engine automatically handles the slanted mask and damage shrinking dynamically. The "Under-Bar" is created as a separate sprite that is explicitly assigned to `@hpBar.viewport` so it inherently adopts the same clipping mask and is placed directly behind the active bar (`@hpBar.z - 1`).
