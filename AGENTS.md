...
- **Method Verification**: You must never assume a class property, getter, or setter exists. Before manipulating any vanilla Pokémon Essentials object (like `Pokemon`, `NPCTrainer`, `Battle`, etc.), you must search the decompiled scripts in the workspace (e.g. `Data/Scripts/`) to verify the exact method names and expected arguments.
