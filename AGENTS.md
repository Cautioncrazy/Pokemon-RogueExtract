...
- **Implementation (`Plugins/Roguelike_Extraction/012_Alpha_Boss_Visuals.rb`):**
  ...
  - **Animated Sprite Overlays:** The Alpha boss visual pattern (`alpha_pattern`) avoids DBK pattern system (`Shadow Pokemon.rb`) entirely for the main battler due to lethal `SystemStackError` cross-plugin recursion loops. It implements a fully standalone architecture utilizing a dedicated floating `@alpha_aura_sprite` lazily instantiated and independently synced inside `BattlerSprite#update`. Generic `Sprite` hooks are retained strictly to cover secondary UI elements (e.g., party icons).
